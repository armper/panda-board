...
import com.panda.board.config.KafkaProperties;
...
class PandaBoardKafkaResourceIT {
    ...
    void setup() {
        ...
        PandaBoardKafkaResource kafkaResource = new PandaBoardKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/panda-board-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/panda-board-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
